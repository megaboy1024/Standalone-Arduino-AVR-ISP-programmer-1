...
byte * readImagePage (byte *hextext, uint16_t pageaddr, uint8_t pagesize, byte *page)
{
  ...
  while (1) {
    uint16_t lineaddr;
    //uint18_t lineaddrold;// to record the next line address , if it's not the next one the clear out the buffer
    ...
    Serial.print("hextext =  0x"); Serial.println(*beginning, HEX);
    Serial.print("Page index =  0x"); Serial.println(page_idx, HEX);
#endif

    //if (lineaddr >= (pageaddr + pagesize)) {
    if (lineaddr != (pageaddr + page_idx)) {
      //hextext = nullptr;
#if VERBOSE
      Serial.print("\nNot same Line address =  0x"); Serial.println(lineaddr, HEX);
      Serial.print("Page address =  0x"); Serial.println(pageaddr, HEX);
      Serial.print("hextext =  0x"); Serial.println(*beginning, HEX);
      Serial.print("Page index =  0x"); Serial.println(page_idx, HEX);
#endif
      if (page_idx == 0) {
#if VERBOSE
        Serial.print("Empty Page address =  0x"); Serial.println(pageaddr, HEX);
#endif
        // 'empty' the page by filling it with 0xFF's
        for (uint8_t i = 0; i < pagesize; i++)
          page[i] = 0xFF;
